...
# Define a custom color palette with color name mappings
...
# Function to capture and return the hex color code of a pixel at specified coordinates
...
# Function to find the closest color name and hex code based on RGB values
def find_closest_color(hex_color):
    ...
    for hex_code, color_name in CSS3_HEX_TO_NAMES.items():
        rgb_reference = webcolors.hex_to_rgb(hex_code)
        color_distance = sum((a - b) ** 2 for a, b in zip(rgb_color, rgb_reference))

        if color_distance < min_color_distance:
            min_color_distance = color_distance
            closest_color_name = color_name
            closest_color_hex = hex_code

    return closest_color_name, closest_color_hex

# Define the key combination to trigger color capture
trigger_keys = ["ctrl", "shift", "alt", "k"]

# Flag to track if the trigger keys are pressed
trigger_pressed = False

# Register a hotkey to detect the trigger key combination
keyboard.add_hotkey("+".join(trigger_keys), lambda: capture_color())

# Function to capture color, find the closest color, and display it when the trigger keys are pressed
def capture_color():
    ...
    if not trigger_pressed:
        ...
        color = get_hex_color(x_coordinate, y_coordinate)
        if color:
            closest_color_name, closest_color_hex = find_closest_color(color)
            print(f"Color at ({x_coordinate}, {y_coordinate}): {closest_color_name} ({closest_color_hex})")
        else:
            print("Error: Unable to capture color.")
        trigger_pressed = False

print("Color grabber is running. Press Ctrl+Shift+Alt+K to capture color.")

# Start listening for keyboard events
keyboard.wait("esc")  # You can use any key to exit the program when done
